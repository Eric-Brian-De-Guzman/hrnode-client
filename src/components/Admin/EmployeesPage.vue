...
<template>
  ...
  <div class="flex justify-between">
    <div>
      <button
        class="text-xs bg-csv hover:bg-blue-700 text-white font-bold py-2 px-4 my-2 rounded-l-lg border-solid border-white-500 border-2"
      >
        COPY
      </button>
      <button
        class="text-xs bg-csv hover:bg-blue-700 text-white font-bold py-2 px-4 my-2 border-solid border-white-500 border-2"
      >
        CSV
      </button>
      <button
      ...
      <button
        class="text-xs bg-csv hover:bg-blue-700 text-white font-bold py-2 px-4 my-2 rounded-r-lg border-solid border-white-500 border-2"
      >
        PRINT
      </button>
    </div>
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg max-h-144">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 overflow-auto">
      <TableHead />
      <tbody>
        <TableEntry v-for="(employee, index) in employees" :key="index" :employee="employee" />
      </tbody>
    </table>
  </div>
</template>
